...
void USART3_Init(unsigned int baudrate){
	...
	PORTB.PIN2CTRL = 0x08;    //Enable the pull up
...
//Interrupt service routine for when data for transmit is empty 
ISR(USART3_DRE_vect){
...
void USART3_Transmit(unsigned char data){
	...
	//USART3_CTRLA |= (1<< USART_DREIE_bp);   //Where I moved the USART3_CTRLA |= (1<< USART_DREIE_bp);
	USART_TxBuf[tmphead] = data;   //Store data in buffer
	USART_TxHead = tmphead;     //Store new index
	USART3_CTRLA |= (1<< USART_DREIE_bp);
