...
int main(void)
{
	...
	*/
	...
	//	temp_in = 0xFF;  //Initialize the LED to be off
		...
		//Canonical sum of products of the 3 input truth table and store that into7
	//	data.bvals.bit6 =  (((~data.bvals.bit7) ^ (~data.bvals.bit6) ^ (data.bvals.bit5)) ^) | (((~data.bvals.bit7) ^ (data.bvals.bit6) ^ (~data.bvals.bit5)) ) | (((data.bvals.bit7) ^ (~data.bvals.bit6) ^ (~data.bvals.bit5))  ) | (((data.bvals.bit7) ^ (data.bvals.bit6) ^ (data.bvals.bit5)) );
